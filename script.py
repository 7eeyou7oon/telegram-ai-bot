...
def save_to_db(name, phone, email, service, source, visit_date, visit_time):
...
def init_db():
    ...
    try:
        cursor.execute(
            "ALTER TABLE requests ADD COLUMN visit_date TEXT"
        )
    except:
        pass
    ...
    try:
        cursor.execute(
            "ALTER TABLE requests ADD COLUMN visit_time TEXT"
        )
    except:
        pass
    conn.commit()
...
def get_requests(limit=10):
    ...
    cursor.execute("""
        SELECT  created_at, source, name, phone, email, service, visit_date, visit_time
        FROM requests
        ORDER BY id DESC
        LIMIT ?
    """, (limit,))
...
def get_requests_by_date(date_from, date_to):
    ...
    if "service" not in columns:
        cursor.execute("ALTER TABLE requests ADD COLUMN service TEXT")

    if "visit_date" not in columns:
        cursor.execute("ALTER TABLE requests ADD COLUMN visit_date TEXT")

    if "visit_time" not in columns:
        cursor.execute("ALTER TABLE requests ADD COLUMN visit_time TEXT")
...
def export_to_excel(data):
    ...
    ws.append(["Дата создания", "Источник", "Имя", "Телефон", "Email", "Услуга", "Комментарий", "Дата визита", "Время"])
...
knowledge_base = {
    "Ремонт ноутбуков": {
        "text": "Ремонт ноутбуков от 10 000 тг. Срок: 1-3 дня.",
        "aliases": ["ноут сломался", "починить ноут", "ремонт лэптопа"]
    },

    "Чистка пк": {
        "text": "Чистка ПК — 2000 тг.",
        "aliases": ["почистить компьютер", "пк грязный"]
    },

    "Диагностика": {
        "text": "Диагностика бесплатно при ремонте.",
        "aliases": ["проверка устройства", "диагностика техники"]
    },

    "Обмен старых ноутбуков": {
        "text": "TRADE-INN — обмен старых ноутбуков. Цена не указана.",
        "aliases": ["обмен ноут", "трейд ин", "сдать ноут"]
    },

    "Выкуп ноутбуков": {
        "text": "Выкуп ноутбуков - цена зависит от состояния.",
        "aliases": ["продать ноут", "скупка ноут"]
    },

    "Обмен ноутбуков": {
        "text": "Обмен ноутбуков: 20 000 – 150 000 тг.",
        "aliases": ["обменять ноут"]
    },

    "Системные блоки": {
        "text": "В наличии системные блоки — от 30 000 тг.",
        "aliases": ["компьютеры", "системник"]
    },
...
async def export(message: types.Message):
    ...
    telegram_data = get_requests(1000)

    site_data = get_site_orders()

    all_data = telegram_data + site_data
...
async def chat(m: types.Message):
    ...
    if uid in user_contacts:
        ...
        if step == "time":
            ...
            save_to_db(name, phone,"-", service,"Telegram", date, time)
